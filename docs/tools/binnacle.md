...
```text
binnacle 0.6.0 -- the housing, and what is in it

  why-slow   0.6.0  why is this box slow?
  agree      0.6.0  which hosts in this fleet disagree with the rest?
  logtriage  0.6.0  which ten lines of this log matter?
  netmesh    0.6.0  is it the network, and which link is sick?
  reachable  0.6.0  which entries in this server list are still real?
  resolve    0.6.0  is it DNS, and which resolver is wrong?
  during     0.6.0  what limited this run, and can you trust the number?
...
```text
  SKEW     netmesh says 0.5.0; this command says 0.6.0.
